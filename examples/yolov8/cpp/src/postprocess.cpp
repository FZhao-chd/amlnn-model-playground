    ...
    for (int i = 0; i < grid_h; ++i) {
        for (int j = 0; j < grid_w; ++j) {
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                for (int t = 0; t < 16; ++t) {
                    exp_vals[k] += t * (exp_logits[t] / sum_exp);
                }
            }
...
std::tuple<cv::Mat, float, std::tuple<int, int>> preprocess(cv::Mat img, std::tuple<int, int> new_shape) {
    ...
    if (img.channels() == 4)
        ...
        cv::cvtColor(img, img_rgb, cv::COLOR_BGR2RGB); // Assume BGR input from imread
    else
        img_rgb = img.clone();
    ...
    cv::Mat img_float;
    img_padded.convertTo(img_float, CV_32F);
    img_float = img_float - 128.0;
    
    img_float.convertTo(img_int8, CV_8S);

    return std::make_tuple(img_int8, scale, std::make_tuple(pad_left, pad_top));
}
